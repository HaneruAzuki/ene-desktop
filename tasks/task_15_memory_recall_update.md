...
| 追加物 | 用途 | 配置 | 注意 |
|--------|------|------|------|
| ローカル埋め込みランタイム（onnxruntime-node 等） | クエリ/要約のベクトル化 | `dependencies` | ネイティブ依存・サイズ要試算 |
| 多言語埋め込みモデル（小型・日本語対応・MIT/Apache） | 同上 | **別ダウンロード**（コア100MB非汚染） | ライセンス精査（無料配布可） |
...
7. **埋め込み＆ベクトル索引**（`src/memory/index-vector.ts` 新規）
   - 書き込み/更新時に `summary` をローカル埋め込み→ `data/memory/{char}/index/vectors`（増分・summary 変化時のみ再計算）。
   - 読み出し時はクエリのみ埋め込み→コサインで上位候補。
