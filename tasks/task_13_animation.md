...
## 申し送り(2026-06): 2D立ち絵アセットを先行投入(付録B-1 VRoid を置き換え)

> 本タスク着手前に、キャラ立ち絵を **2Dルート**(Fooocus原案 → Nano Banana 2 で表情差分 →
> rembg/BiRefNet 透過 → Photopea 手仕上げ)で作成・配置済み。**付録 B-1(VRoidスプライト作成)は
> この2Dアセットで置き換える**(VRoid は作らない)。データ駆動(`animation.json` 差し替えで反映)の方針は不変。

### 配置済みアセット(`characters/ene/` 直下)
- 全10枚・**832×1281・透過PNG・全身・両手ポケット・正面**。全フレームが**単一の透過ボディを共有**(体・縁・首が完全一致=切替時のチラつき無し)。
- 命名(暫定): `portrait.png`(neutral・口閉じ) / `portrait-talk.png`(neutral・口開き) /
  `portrait-{happy,dere,tsun,sad}.png`(口閉じ) / `portrait-{happy,dere,tsun,sad}-talk.png`(口開き)。
- **`sprites/` ではなく `characters/ene/` 直下**に置いてある。task_13 実装時に `sprites/` へ移設し `animation.json` を記述する想定(命名は本タスクの `frames` 規約に合わせて確定)。

### emotion ラベル対応(本タスク `EMOTION_LABELS` 6種への写像)
| 制作ラベル | task_13 ラベル | base(閉) / baseOpen(開) |
|---|---|---|
| neutral | `neutral` | portrait.png / portrait-talk.png |
| happy | `joy` | portrait-happy.png / portrait-happy-talk.png |
| tsun | `anger` | portrait-tsun.png / portrait-tsun-talk.png |
| sad | `sorrow` | portrait-sad.png / portrait-sad-talk.png |
| dere | `embarrassed` | portrait-dere.png / portrait-dere-talk.png |
| (未制作) | `surprise` | **無し** → 当面 `neutral` にフォールバック(F-ANIM-06)。必要なら後日2Dで1枚追加 |

- 口開き(`-talk`)は **`map.baseOpen`**、閉じは **`map.base`** に対応。口パクは閉⇄talk の交互(F-ANIM-05)。
  `-talk` は **「口の箱(X 397–439 / Y 237–267)」だけRGB差し替え・他は透明度ごと閉と完全一致**で作成済み(切替で動くのは口だけ)。
- **未制作フレーム**: `thinking`(考え中)・`sofa`(寝そべり)・`surprise`(驚き)。プレースホルダ(neutral 流用)で開始し、必要なら後日追加。

### 表示側の要対応(重要)
- 現 `CharacterDisplay` の `.character` は **240×320・object-fit 未指定(=fill)**。本アセットは**全身・縦横比≈0.65**なので、このままだと**横に潰れて歪む**。
  → `frameSize` を実寸(832×1281)基準にし、**`object-fit: contain`** ＋ **ウィンドウ/表示寸法の再設計**が必要。別添A.5 の 240×320 は半身プレースホルダ前提で全身には不適。
- 設計判断4「`sofa` を 240×320 枠内」「ウィンドウ動的リサイズしない」も、**全身立ち絵前提で寸法を見直す**こと。

### ルート変更の含意(ロードマップ)
- 素材が **2D(VRoid でない)**ため、将来の本格モーションは **VRM/three-vrm ではなく Live2D 路線**が自然(`research-image-pipeline` 結論)。`docs/00_philosophy.md` の 1.0(Live2D/VRM)の選択に反映を検討。
