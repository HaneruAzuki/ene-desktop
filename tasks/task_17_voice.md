...
| **B 入力(STT)** | mic取得/STTエンジン実装/確定テキスト→既存`sendMessage`経路/STTモデル初回DL | 不要 |
| **C 双方向・ターン** | Silero VAD/Smart Turn v3.2/barge-in/4層防御×streaming(C2)/listening-thinking-talking 状態機械 | 不要 |
