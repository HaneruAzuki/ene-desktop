...
- ✅ `02_requirements.md`:**§2.14 F-VOICE-01〜11 を追加**(2026-06-08・N-17-11)
- ✅ `03_design.md`:**§1.2(音声ライブラリ=新規npmなし)・§2(音声モデル/声設定/app-settings の配置)・§3.4(`reading?`＋音声化フロー)・§4.2(音声IPC)・§11.1(実装済みマーク)を反映**(2026-06-08・N-17-11)
- ⚠️ 注記:**C1 ストリーミング応答型はライブ未配線**(完成JSON→`speakText` の非ストリーミングで実装)。正本もこの実態で記載。Phase D(声/レイテンシの人間判定)と振幅リップシンクは未完。
