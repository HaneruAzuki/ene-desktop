...
前者はユーザPCで動くコードに含まれ、後者はビルド時にのみ使用される。
...
> - **キャラVRM本体**(`{id}/*.vrm`=`ene/torimi.vrm`・約3〜10MB)は portrait.png 同様 **exe へ同梱**(`ene/**` は既に files/asarUnpack 対象)。whisper/ruri のような任意・別DLアセットとは区別する(表示に必須のため)。小窓表示前提でテクスチャ1K・マテリアル削減により最適化。
...
> 使わない・MVP では不要)。将来 Fast Refresh が必要になったらユーザ承認のうえ
...
>   エンジン/声モデルはユーザ領域に配置(同梱しない=コア<100MB維持)。声は寛容ライセンス(つくよみコーパス基盤の自作AIVM)。
...
- メジャーバージョンの更新は、必ずユーザ承認を経た上で本表を更新

**重要**:上記以外のライブラリを追加する場合は、必ずユーザ承認を得ること。

> 📌 **この表は CLAUDE.md §2 から参照される「技術スタックの唯一の真実の源」である。**
> ここを変更したら、その変更がプロジェクト全体に直ちに反映される。
> 追加・削除・バージョン変更時は、必ずユーザ承認を経た上で本表を更新すること。
...
- **Renderer はマルチエントリ構成**:メインUI(`src/app/renderer/`)に加え、APIキー管理ダイアログ
  専用ページ(`src/app/renderer/api-key-dialog/`)と専用 preload を持つ。electron-vite の
...
> 新規ディレクトリの追加・既存ディレクトリの削除・役割変更は、必ずユーザ承認を経た上で本ツリーを更新すること。

> 構成原則(2026-06 再編):`src/` 直下は**ドメイン名詞**(character / knowledge / memory /
> conversation / voice)+**土台**(app / shared)。トップレベルが「何のアプリか」を叫ぶ
> (Screaming Architecture)。各ドメインと哲学「四つのあり方」の対応表・1ターンの流れ図は
> `docs/05_architecture.md` を参照。キャラ定義はルート直下 `ene/`(characterId がそのまま
> ディレクトリ名。ENE=コードネーム・CLAUDE.md §5.1)。
...
│   ├── 04_positioning.md           ← 外向き(市場・言葉)の正本(系譜・コピー・配布・投げ銭)
│   ├── 05_architecture.md          ← 体(フォルダ)×魂(哲学)の対応表・1ターンの流れ図
│   ├── A_character_profile_samples.md ← キャラJSONサンプル
│   ├── B_dependency_license_audit.md  ← 依存ライセンス監査
│   ├── character-life-memory-canon-plan.md ← 人生記憶 canon の内容計画
│   ├── handoff-vrm-character-display.md ← VRM 3D化の引き継ぎメモ(大半実装済み・経緯)
...
│   ├── voice-ab.mjs               ← 音声 A/B テスト(読み間違い対策の検証・task_17)
│   ├── stt-smoke.mjs              ← STT スモーク(task_17・手動検証)
│   ├── vad-smoke.mjs              ← VAD スモーク(task_17・手動検証)
│   ├── vad-fragment-test.mjs      ← VAD 分断の原因究明ハーネス(一時診断ツール)
│   ├── vad-load-sim.mjs           ← VAD 負荷シミュレーション(一時診断ツール)
│   ├── filler-audition.mjs        ← 思考フィラー/相槌候補の試聴用合成(一時ツール)
│   ├── vrm-smoke.mjs              ← VRM 計測ハーネス(常駐CPU/RAM 実機検証・F)
│   ├── vrm-harness.html           ← vrm-smoke 用の描画ページ
...
│   │       ├── use-ene-events.ts      ← main(ene)→renderer の全イベント購読(可視性導出含む)を集約したフック
...
│   ├── backchannels.json          ← 相槌・思考フィラーの語彙(task_18・任意)
│   ├── vrm.json                   ← VRM 表示設定(F・任意・無ければ 2D 立ち絵)
│   ├── torimi.vrm                 ← 3D モデル(F)
...
│   │   └── *.test.ts              ← character/knowledge/memory/conversation/voice/shared 等の純粋ロジック
...
// ユーザが当日に祝ったら history に celebrated: true を記録
```
...
- ユーザの反応に対する具体的なキャラの応答は、**すべて Few-shot で表現**する
...
> **完全ローカル・ネットワーク0往復**のハイブリッド判別(`src/knowledge/local-classifier.ts`
...
> (tech=得意げ / 賭博=困惑 / 危険=拒否)が復活**した。旧 Haiku 版 `classifyTopic` は
> B-15 置換後に**死蔵コードとして削除済み**(2026-06-12・下記「旧 Haiku 版」参照)。
> 詳細は下記「ローカル判別器」と `implementation-notes.md` N-LAT-9。
...
```typescript
// src/knowledge/local-classifier.ts
...
#### 旧 Haiku 版 `classifyTopic`(削除済み・経緯のみ)

> 📌 初期実装は Haiku(`claude-haiku-4-5`)へのネットワーク往復で判定する `classifyTopic`
> (800ms タイムアウト・LRU キャッシュ・router.ts / cache.ts / response-parser.ts)だった。
> 実機では往復が 800ms を超え実質常に fallback=medium になっていた(N-09-9)ため、
> 本番経路をローカル判別器(B-15・上記)へ置換し、Haiku 版一式は**死蔵コードとして
> テストごと削除した**(2026-06-12)。将来のマルチプロバイダ(§11.7)等で再導入する場合は
> git 履歴から復元する。知識境界は buildSystemPrompt 側にも含まれるため、どの経路でも
> fallback 時に「知らない」応答は担保される。

### 3.3 Memory Layer

#### 責務
- 3層記憶(短期・中期・長期)の管理
- 想起(task_15):ユーザ発言を引き金に**全件横断**で引く(語彙＋entity＋ベクトルの RRF・Router 非依存)
...
export interface OsCommandResult {
  ok: boolean;
  message?: string;          // ユーザに見せるキャラ口調のメッセージ(失敗時は reason 由来)
  reason?: OsFailureReason;  // ログ用の失敗理由(ユーザには見せない)
}
```
...
| `open_folder` | 開きたいフォルダの絶対パス | 必須 | ユーザホーム配下のみ許可 |

#### ホワイトリスト・検証ロジック

```typescript
// src/app/main/os/validators.ts
...
| ユーザホーム外アクセス | ホームディレクトリ配下に限定。`C:\Windows` 等を防ぐ |
...
// src/shared/node/json-store.ts (平文JSON操作)
...
- ユーザが直接 `data/memory/{characterId}/semantic.json` を開いて中身を確認できる

#### 会話の逐語ログは保存しない

- ユーザの会話内容を逐語的に記録するログ機能は**実装しない**
...
- ユーザがキーを後から変更する手段の提供
- Anthropic API キー取得手順のユーザ案内(I17)
...
```typescript
// src/app/main/api-key-dialog.ts(実装方針)
...
- **N-09-1**:ダイアログ用 IPC(test/save/open-console/close)はモジュール状態(現在の窓・onSaved・close 結果)
  を共有する必要があるため `src/app/main/api-key-dialog.ts` に**統合**する。グローバルハンドラは一度だけ登録(再登録エラー回避)。
...
export interface BirthdayHistoryEntry {
  year: number;                  // 西暦
  celebrated: boolean;           // ユーザが誕生日に触れたか
...
└── ene/                         ← 固定キャラ(魚川トリミ)の記憶。characterId は "ene"
    ...
    （人生記憶 canon は ene/life-memory.json 側＝キャラ資産・読取専用）
```
...
- 「ENEもTakeshiも、ユーザのことは共通で覚えていてほしい」というニーズが
...
本アプリはユーザ個人の PC で動く。ユーザから見える時刻表現はすべて
**そのユーザのローカルタイム**で統一する方が直感的。

- **誕生日判定**:ユーザから見て「8月15日 0:00〜23:59 ローカル時間」が誕生日
  - UTC で `2026-08-15T00:00:00Z` を使うと、日本では「8月15日 9:00〜8月16日 8:59」がズレて誕生日扱いになる
- **記憶の日付**:「昨日話したこと」はユーザの体感日付で記録すべき
- **ログ**:ユーザが調査する時、ローカル時刻表記の方が即座に理解できる
...
> 📌 **重要**:いずれのレベルでも、**ユーザ入力・AI応答・プロンプト全文・記憶内容
...
> - 起動シーケンスは `runStartupSequence(runtime)` に集約(`src/app/main/lifecycle.ts`)。
...
> ウィンドウ拡大や吹き出し表示時の動的リサイズは MVP 後のブラッシュアップとする(ユーザ判断済み)。
...
```typescript
// src/app/renderer/components/mouse-constants.ts
...
| 表示から 30 秒経過 | 自動消滅 |
| ユーザが新しい入力を送信 | 古い吹き出しを即座に消去(新応答が次に表示される) |
| 吹き出し本体をクリック | 即座に閉じる |
| ESC キー押下 | 即座に閉じる |
...
```typescript
// src/app/renderer/components/bubble-constants.ts
...
```typescript
// src/app/renderer/components/SpeechBubble.tsx(疑似コード)
...
ユーザがクリックすると、デフォルトでは ENE のウィンドウがイベントを
...
```typescript
// src/app/renderer/components/CharacterDisplay.tsx(疑似コード)
...
初見ユーザが「どうやって話しかけるの?」で詰まらないよう、ENE 自身が
...
ユーザがアプリを終了する手段として、**タスクトレイメニューと
キャラクター右クリックメニューの2系統**を提供する。

#### タスクトレイアイコン

```typescript
// src/app/main/tray.ts(実装方針)
...
```typescript
// src/app/renderer/components/CharacterDisplay.tsx(実装方針)
...
| 項目 | 採用 | 理由 |
|------|------|------|
| タスクトレイ | ✅ | Windowsユーザが常駐アプリの終了を探す第一候補 |
...
- ただしキャラ依存値の JSON 外出し・`{id}/` 構造は**クリーンさ・可逆性のため維持**する。
...
- ユーザがプロバイダを選択する UI は、設定画面拡張(将来)と併せて実装
...
- ✅ パス操作は必ず `src/shared/node/paths.ts` 経由
- ✅ エラーメッセージは日本語で書く
