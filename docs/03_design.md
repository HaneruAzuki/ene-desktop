...
│   │   └── api-key-dialog.ts      ← APIキーダイアログ専用 preload(N-09-2)
│   │
...
export interface RouterResult {
  ...
  matchedTopic?: string;   // 一致したトピック(任意・会話時の Episodic 検索タグに使う・§3.4 N-07-3)
...
```typescript
// src/router/classifier.ts(実装は classifyTopic)
...
export function searchEpisodic(query: MemorySearchQuery): Promise<EpisodicMemory[]>;
...
- **N-07-3**:会話時の Episodic 検索は `buildMemoryContext({ tags: matchedTopic ? [matchedTopic] : undefined, limit: 5 })` の簡易タグ一致(意味検索は §11.4 で将来)。
