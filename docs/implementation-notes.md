...
- **⚠️ 2026-06 是正**: 当時の「速さは追わない/リアルタイム性は抑える対象」という根拠は**誤り**(哲学 §1.4 共通の大原則で是正)。決定(双方向ローカル音声)は維持しつつ、**速さは常に最大化**する。
...
### N-LAT-7 🟢 第一声(ストリーミング)を内訳計測 → TTFT 律速と判明・施策A/VAD短縮で着地
- **該当**: `sentence-splitter.ts`(`splitFirstChunk`)・`json-stream-parser.ts`(第一チャンク早期 drain)・`constants.ts`(`FIRST_CHUNK_MAX_CHARS=20`・`VAD_MIN_SILENCE_MS` 500→350・`BACKCHANNEL_PAUSE_TRIGGER_MS` 400→300)・`voice-runtime.ts`(計測は撤去済)。
- **施策A(早期チャンク)**: 第一声を早めるため、**最初のチャンクだけ**文末を待たず読点(、)/改行/字数(20)で区切る。ルビ`《…》`は分断しない(基底+ルビが割れないようガード)。2文目以降は通常の文単位。
- **内訳計測(実機・施策A後・一時的に計装して撤去)**: 第一声 ~2.4s の内訳 = **TTFT(最初のトークン)~1500ms(約65%)** / 生成→第一チャンク 23〜417ms(**Aが効く時は23-83ms**) / TTS合成 ~450ms。→ **第一声は TTFT 律速**(ネットワーク往復＋約2500トークンのプレフィル=クラウドの床)。
- **結論(レバーの当否・ユーザーと確認)**: A は機能するが TTFT が支配的で総量は不変。**E(JSONキー順=前置き短縮)は前置き生成が既に~50ms以下=実質無意味**。入力削減=①episodic想起5→3(新規トークン・唯一クリーンだが ~100-200ms)/②ShortTerm要約圧縮(**スライド窓で毎ターン要約が変わりキャッシュを壊す=賭け・不採用**)/③fewshot間引き(**キャッシュ済=削っても TTFT 効果小・口調リスク=却下**)。→ **第一声は実質的に床。素直な残りは①のみ**。
- **VAD 終話短縮**: `VAD_MIN_SILENCE_MS` 500→350(体感 -150ms)。不変条件「相槌スロット< 終話」を保つため `BACKCHANNEL_PAUSE_TRIGGER_MS` も 400→300 へ連動。**リスク=遅い発話の語尾取りこぼし＋相槌わずかに前のめり=実機ユーザー判定**。
- **相槌の文末発火(B-17)**: 調査中に「文末でも相槌が出る」を発見。**遅延原因ではない**(終話判定は独立・相槌音声は別キュー `ene:backchannel`/`playBackchannel`)。ユーザー判断で**今回見送り**→ `optimization-backlog.md` B-17(本命=「発話再開時に撃つ」方式)。
- **検証**: typecheck/lint/全422テスト緑(splitFirstChunk・早期チャンクの回帰テスト追加)。

### N-LAT-8 ⚪ ストリーミング/可変長STT を調査 → ライセンス/精度で見送り(whisper-small が床と再確認)
- **背景**: 第一声の残ボトルネックのうち STT(~800ms)を「聞いている時間」に畳めないか(ユーザー要望「ストリーミングSTTの可能性」)。
- **調査結論(2026-06-09・WebSearch/WebFetch)**:
  - **真の逐次(online)日本語ASR**は稀。sherpa-onnx の online モデルに日本語なし(bn/zh/ko/en)。ReazonSpeech-ja は**オフライン**(VAD+一括=現状と同型・利得なし)。
  - **Vosk-ja**:唯一の真ストリーミング日本語・Apache-2.0(クリーン)・Node。だが**精度が whisper より明確に低い**(Kaldi世代)=精度ダウングレード＋新ネイティブ依存。
  - **Moonshine(可変長ASR・本命候補)**:whisper の30秒pad固定と違い音声長比例で高速(~107msクラス)。日本語モデルあり・**transformers.js 公式サポート=現スタックで near-drop-in**・インクリメンタル対応。**だがライセンスが致命的**=コード/英語モデルは MIT だが**非英語(日本語)モデルは独自「Moonshine Community License」**(登録義務・"Powered by Moonshine AI" 表示強制・年商$1M上限・競合開発禁止・将来失効リスク)。**非商用限定ではない**が**寛容ではない**=ENE の「完全ポータブル・紐なし」哲学に不適。**ユーザー判断で見送り**。
  - **whisper-streaming(チャンク式)**:精度は保つが発話中に whisper を反復実行=CPU持続消費で「軽量・3%」と衝突。
...
### N-VRM-3 ⚪ ターン終端うなずき=ターンテイキングの視覚信号(2026-06-12・ユーザー設計)
- **狙い**: ハンズフリーで「無音窓が閉じた=トリミがターンを受け取った」ことを、**音を増やさず**(フィラー音声はうっとうしいと判断)**うなずきで視覚的に**示す。これにより無音枠終端の位置がユーザーに分かる。
- **発火点**: `VadRuntime.endTurn()`([src/main/vad-runtime.ts](../src/main/vad-runtime.ts))=無音窓が閉じた瞬間。STT を待たず**窓終端ぴったり**で出る(レイテンシ追加ゼロ)。新チャネル `ene:turn-nod`(strength)で renderer へ。
- **出し分け**: 深さ=**直前の発話の長さ(録音フレーム数→ms)**で2段階(`turnNodStrength`・純粋関数)。境目 `TURN_NOD_LONG_THRESHOLD_MS=10000`(≤10秒=情報量が少なく即理解=軽い `0.4` / >10秒=情報量が多く一拍考える所作=深め `0.8`)。**文字数でなく秒数**にしたのは、文字数だと STT 完了(~1.8s)まで待ち窓終端から遅れるため。
- **表現**: 既存うなずき機構を深さ・速さでパラメータ化。VRM=首ボーン回転量 `0.25 * strength`([vrm-renderer.ts](../src/renderer/vrm-renderer.ts) `nod(strength)`)/ PNG=CSS 変数 `--nod-scale`(`ene-nod` の translateY 倍率)。**速さは全うなずき共通で1.5倍ゆっくり**(VRM 減衰係数 3→2・CSS 0.55→0.83s・「ピョコン」回避)。相槌(聞くターン)のうなずきも `BACKCHANNEL_NOD_STRENGTH=0.4`(ターン終端の浅い側と同程度・1.0 から下げ)。深さ・速さ・境目は全て constants の定数で調整可(2026-06-12 実機で詰めた値)。
...
### N-TURN-1 🟢 Smart Turn v3(意味的終話判定)の採用見送り — 「会話はリズムであって意味ではない」(2026-07-04)
- **決定**: ターンテイキングは**現行のリズム方式(Silero VAD＋適応無音窓＋投機生成＋barge-in)を維持**し、意味的終話判定モデル **Smart Turn v3 は導入しない**。原文 `N-17-1/2/5` および archive `design-revision-voice.md` の「将来 Smart Turn へ upgrade」計画を、本エントリで**破棄**する。
- **Smart Turn の正体**: 音声波形から `P(言い終えた)` を返す**単機能センサー**(Whisper-tiny エンコーダ・8MB・CPU 12〜65ms・BSD・日本語96.76%)。ターンテイキング"システム"ではない。我々の `voice-turn-coordinator`(傾聴・投機生成・連結・追撃処理)は、その上の**方針レイヤー**で、Smart Turn は置き換えない。
- **見送りの理由(本製品構成では限界価値が低い)**:
  1. **投機生成＋コアレッシングが既にレイテンシを吸収**している(Smart Turn がクラウド音声エージェントで解く「無音を待たず応答」を、別解で解決済み)。
  2. **マイク＋スピーカ前提(ヘッドホン非推奨)**=エコー常在で、韻律ベースの意味判定はエネルギーVADより脆い。
  3. **雑談 companion=だらだら発話**で「文の完結 ≠ ターンの終了」。意味endpoint はラン話者を途中で遮る(＝リズム=止まったか、の方が正しいターン合図)。
  4. **短発話主体**なら文単位STTチャンク化の精度/レイテンシ利得はノイズ域(Whisper のチャンク間文脈 `condition_on_previous_text` はドリフトで無効化が定石=失う文脈は元々乏しい)。
- **世間との関係**: 意味endpoint(Smart Turn/LiveKit)はクラウド**タスク型**音声エージェント(電話ボット等)の業界標準。本製品の相違は「ローカルファースト companion」という**別カテゴリ**ゆえで、遅れではなく別解。
- **再考トリガ(将来この条件なら価値が跳ねる)**: (a) クラウド/ストリーミングSTTへ移行 (b) タスク型の使い方が増える (c) コアレッシングを外す。
- **もし再考する場合の先行実測**: `voice-turn-coordinator` の投機中断/stale破棄回数と文中カット率をログし、頻度で要否を判断(思い込みで入れない)。
- **副産物(将来やるなら別立て)**: Smart Turn は"ターンの審判"でなく"文の区切り屋"としてなら低リスクの用途あり=①傾聴中の文末ランダム相槌 ②長尺モノローグの文単位STT。いずれも**本決定の対象外・現時点は不要**。
- **反映**: philosophy §2 表 / vision §6 表 / `B_dependency_license_audit` §2.3 / `constants.ts`(VAD_MIN_SILENCE_MS コメント) / `task_17_voice.md` の Smart Turn 記述を除去・更新。歴史ログ(`N-17-1/2/5`)と archive は記録として残置(本エントリが上書き)。
