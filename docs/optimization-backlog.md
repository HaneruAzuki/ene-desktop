...
- **残 (b)**: 想起・埋め込みを**ワーカースレッド**へ(main のイベントループ/UIアニメを止めない)。
- **残 (c)**: **埋め込みモデルを起動時ウォーム＋クエリ埋め込みをキャッシュ**(初回/毎ターン推論を削る)。
- いずれも品質劣化ゼロのローカル施策。残りは本3点の実機計測後に着手。
...
- **前提(重要・ユーザ案)**: `reading`(ひらがな)は AivisSpeech の漢字読みが弱いための緊急回避策。**AivisSpeech のユーザ辞書API(`/user_dict_word` 等)で固有名詞・キャラ語彙・誤読語を登録**して読みを底上げすれば、`reading` を**廃止**でき、Claude は素の漢字テキストを返すだけになる。→ ストリーミングは**「文で区切って順に合成」**の単純形で済む(以前撤回した行形式の複雑さが不要)。辞書は `characters/{id}/voice-dict.json` 等に外出し(§4.5)し provisioner が冪等登録。
