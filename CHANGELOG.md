...
- 音声(VAD/STT)/ 埋め込みモデルの初期化失敗が再起動まで治らなかった不具合(失敗時に自己回復するよう修正・3経路統一)。
- 非ストリーミング発話でルビ(漢字《よみ》)が読み下されず、TTS が記号や読み仮名を読み上げていた不具合。
- 記憶想起の provenance 混線(canon と user 記憶の取り違え・N-RECALL-1)。
- 短期記憶の取りこぼし: timestamp(秒精度)を識別子に流用していたため同一秒の衝突で抽出マークが巻き添えになる不具合(一意 id 化)。
- ベクトル索引が次元不一致(モデル差し替え・埋め込み失敗の空ベクトル)を黙認し検索精度が静かに劣化する不具合(次元検証＋再構築)。
- ハンズフリー VAD が高負荷時にフレームを無言でドロップしていた不具合(バウンド付きキューでバースト吸収＋計測ログ)。

### 構造・規約

- IPC チャネル名を単一の真実の源(`shared/ipc-channels.ts`)へ集約し、main / preload の綴りズレをコンパイル時に検出。
- 依存方向を dependency-cruiser で機械強制(ドメイン→app 逆流・shared→上位・循環・memory 索引/ストアの層外参照を読み書き両側で禁止)。
- 記憶レイヤーの読み書き窓口を facade 化(`episodic-write` / `presence-reads`)し、ストレージ実装を上位から隠蔽。
- コーディング規約を ESLint で機械強制(`max-lines` §8.5・`explicit-function-return-type` §8.1・キャラ名ハードコード禁止)。
- App.tsx(god-component)を用途別フックへ分離(設定 / 音声入力 / IPC購読 / 当たり判定)。vad-runtime は DI 化しユニットテストを追加。
