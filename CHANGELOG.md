...
- **OS操作機能(OS Integration)**:メモ帳/ブラウザ/フォルダを開く `os_command` 応答型・`open_*` action・
  実行レイヤー・`OsCommand` 型を全廃。トリミは PC を操作する手段を持たず、頼まれても chat 型で「できない」と断る
  (才能はあるが代行はしない=キャラクター設定と整合)。
- **会話ログ(右パネルの履歴表示)**:VTuber 風の履歴パネルと `ene:set-log-expanded` を撤去。
