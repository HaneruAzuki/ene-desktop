...
//
// 抽出は**トリミの記憶**として記録する(2026-06-21 改訂・旧「中立観察者」から転換)。事実は正確に＋
// ユーザーの発言をトリミがどう受け取ったかを会話での実際の反応から一言添える(口調そのものは混ぜない=記録として簡潔)。
// task_15 で2点拡張:
...
const EXTRACTION_SYSTEM = [
  '以下は「ユーザー」と「トリミ(=相手)」の会話です。ここから長期的に意味のある事実・嗜好・出来事を、',
  '**トリミの記憶**として書き留めてください。',
  '- 事実(ユーザーが何を言った/何が起きたか)は正確に記録する。',
  '- そのうえで、ユーザーの発言を**トリミ(=相手)がどう受け取ったか**を、会話でのトリミの実際の反応に基づき一文で',
  '  添えてよい(例「ユーザーが昇進を報告。トリミは強がって『別に』と流した」)。口調そのものは真似ず記録として簡潔に。',
  '  会話にトリミの反応が無ければ受け取りは書かない(捏造しない)。',
  '',
  ...
  '- それ以外は「トリミにとっての印象深さ」で付ける:トリミが会話で強い関心・感情を示した話題(食いついた/喜んだ/',
  '  こだわった)は importance を高め・summary も少し詳しく。逆に興味を示さなかった事務的な話は簡潔・低めでよい。',
  '- category は health / work / hobby / relationship / general などの短い英単語。',
  '- valence は**ユーザーにとっての**出来事の感情的トーン(-2=とてもつらい 〜 0=中立 〜 +2=とてもうれしい)の整数。',
  '  「ユーザーが楽しそうに/つらそうに語ったか」の客観的トーンで、上記の“トリミの受け取り”とは別物',
