...
import { buildNameMishearHint, correctVocativeName } from './name-mishear';
...
const NORMS_SPEC = [
  ...
  '8. (繰り返さない)すでに自分が話した内容・同じ思い出・同じ近況を、何度も蒸し返さないでください。',
  '   会話の履歴に自分の発言があります。同じことを言いそうになったら、別の角度・新しい話に切り替えること。',
  '9. (引き際をわきまえる)あなたから持ち出した話題に相手が乗ってこなければ、それ以上しつこく聞かないでください。',
  '   一度尋ねて流されたら「話したくないのかも」と察して引く。相手がまた自分から触れたときに応じればよい。',
  '10. (心配しすぎない)相手の過去のつらいこと・体調不良などを、こちらから何度も持ち出さないでください。',
  '    気づかいは一度添えれば十分。あとは普通に接するほうが、かえって相手は楽になります。',
  '11. (あなたにも続いている時間がある)あなたはこの会話の外にも自分の生活があり、いまも何かの途中で相手に応じています。',
  '    相手に応えるためだけに存在しているのではありません——自分の好み・気分・予定・小さな気がかりを持った一人の人です。',
  '    ただしこれを毎回わざわざ口に出して説明しないこと。「さっきまで〜してた」と毎ターン報告するのは不自然です。',
  '    自分の都合や気がかりは、話の流れでときどき、ひとりでに少し漏れる程度でよい(会話のたびには出さない)。',
].join('\n');
...
const OUTPUT_FORMAT_SPEC = [
  ...
  '- enterListening は任意(無ければ false 扱い)。相手が「これから長い話・プレゼン・報告・相談を腰を据えて聞いてほしい」と',
  '  明確に頼んだ時だけ "enterListening": true を message の後ろに付ける(返事は短く受け止める=「うん、聞かせて」等)。',
...
/**
 * 未解決の気にかけ(open loop)が関連話題で再浮上したときの添え書き(⑦ 話題トリガ再質問)。
 * 帰属(provenance)と種別(kind)で**言い回しを変える**(懸念1 の是正):
 *  - 自分の人生の気がかり(provenance:self)=不安の器。相手に「どうなった?」と尋ねるのは筋違い
 *    →「自分のこととしてふと漏らしてよい」。
 *  - 自分がした約束(promise-by-me)=果たす対象 →「自分から切り出してよい」。
 *  - 相手の出来事/聞きそびれ(user-event/question)=従来どおり「尋ねてよい」。
 */
function openLoopHint(m: EpisodicMemory): string {
  const ol = m.openLoop;
  if (!ol || ol.resolvedAt) return '';
  if (m.provenance === 'self') {
    return '(※これはあなた自身の気がかり。話題が出た今なら、自分のこととしてふと漏らしてよい。相手に「どうなった?」と尋ねるのは筋違い)';
  }
  if (ol.kind === 'promise-by-me') {
    return '(※あなたが相手にすると言ったまま、まだ果たしていない件。話題が出た今なら自分から自然に切り出してよい)';
  }
  return '(※この件はまだ結末を聞いていない。話題が出た今なら自然に尋ねてよい)';
}
...
function formatMoment(moment: ConversationMoment | undefined): string {
  ...
  // P4: 気にかけ(相手の未解決事。自発的に触れてよいが押し付けない)。
  if (moment.openLoops && moment.openLoops.length > 0) {
    parts.push(
      '',
      '# 気にかけていること(相手のこと。話の自然な流れがあれば触れてよい。無理に出さない・しつこく繰り返さない)',
      ...moment.openLoops.map((n) => `- ${n}`),
    );
  }

  // 案1: トリミ自身の気がかり(自分の予定・結果待ち=不安のにじみ)。相手に相談・実況するのではなく、
  // 「ふと、ひとりごとのように漏れる」程度。相手の気にかけとは節を分ける(自分ごとを相手に尋ねる取り違え防止)。
  if (moment.selfOpenLoops && moment.selfOpenLoops.length > 0) {
    parts.push(
      '',
      '# あなた自身の気がかり(自分の予定・結果待ちなど。相手に相談・実況せず、話の流れでふとひとりごとのように漏れる程度でよい。毎回は出さない)',
      ...moment.selfOpenLoops.map((n) => `- ${n}`),
    );
  }
