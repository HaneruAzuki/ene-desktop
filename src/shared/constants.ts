/* eslint-disable max-lines -- 全レイヤーの定数表(§4.5 マジックナンバー一元化)。各値のコメントが
   設計意図そのもので、分割すると意味の対応が散る=分割しない正当な例外(§8.5)。 */
// アプリ横断の定数(設計書 §3.3 ほか)。
// マジックナンバーを散らさず一元管理する(CLAUDE §4.5)。

/** 1日のミリ秒(日数換算の共通定数・相手のトーン/親しさの導出で共用)。 */
...
// **常時オン**(2026-06-13 ユーザ決定・実機検証済 N-FORGET-1)。忘却=「人間らしい忘却」は製品同一性そのものなので、
// トグルで切れる設定は持たない(切れると"忘れない不自然な存在"になり同一性を否定する・2026-06-24 設定棚卸し)。
...
/**
 * VAD フレームの取り込みキュー上限(フレーム数・約1秒相当)。
 * Silero は RNN 状態を持つため逐次処理が必須で、推論が一時的にフレーム間隔(約32ms)を
 * 超えるとフレームが溜まる。バーストはキューで吸収して取りこぼしを防ぎ、持続的に
 * 処理が追いつかない場合のみ最古を捨ててバックログを有界に保つ(発話頭の欠落と無限遅延の両方を防ぐ)。
 * 16kHz/512サンプル ≒ 31.25 フレーム/秒。
 */
export const VAD_FRAME_QUEUE_MAX = 32;
...
/**
 * 「第一声(=コミット)が出るまで」の監視上限(ms・N-LAT-10)。これを超えても第一声が出ない=生成がハング/
 * 失敗とみなして ctrl.abort() で打ち切る。TURN_TIMEOUT_MS(全体60s)とは別の、**ターン冒頭の短い救済**:
 *  - 60s も「考え中」を見せ続けない(声の会話には長すぎる)。
 *  - 諦めた後に Claude が遅れて返って「急に喋り出す」事故を防ぐ(レンダラの考え中ウォッチドッグより前に切る)。
 * 第一声後の後続合成の長さは TURN_TIMEOUT_MS 側で見る(正当な長文応答を殺さない)。
 */
export const FIRST_AUDIO_TIMEOUT_MS = 12_000;
/**
 ...
 * 履歴: 320(エコー対策の実機調整値)→500(2026-06-13 傾聴モード・長答を事故被りで失わない)
 *  →220(8101fa1・barge-in 機敏化の実験)→**500 へ戻す(2026-06-23・外付けスピーカーで自声エコーが
 *  barge-in を誤発火し「トリミが自分の声に返事する」事故の対策)**。
 * トレードオフ: 小=割り込みやすいがエコー誤発火しやすい / 大=エコーに強いが割り込みに発話継続が要る。
 * 外付けスピーカーは AEC を擦り抜けて自声が回り込むため短いと誤発火する。実機調整可(500〜800 が目安)。
 */
...
// --- 想起の個性化(関心アフィニティ)・開示ゲーティング ---
// recall の個性は「関心アフィニティ=トリミの関心に触れる記憶を優先想起(自分の関心事に飛びつく)」で出す。
// 旧「元気づけ(valence の減衰平均 recentUserTone → 正valence記憶を加点)」は撤去(2026-06-24・③b)。
// 複雑な割に体感に出ず、「落ち込み」検出は履歴の数値平均でなく**現在の会話の cue**(mood-cues.ts)で行い、
// 想起バイアスでなく moment ヒントで「明るい話題にそっと触れてよい」と促す方式へ簡素化した(context-builder)。

/** 関心アフィニティの加点(トリミの関心に触れる記憶へ)。RRF と同オーダー(調律可)。 */
export const INTEREST_AFFINITY_WEIGHT = 0.01;

/**
 * 想起の softmax サンプリング温度(大きいほど揺らぎ・小さいほど上位集中・調律可)。
 * 経緯: 当初 0.01 で「上位~20候補をほぼ一様抽出して無関係が混ざる」問題があったが、それは候補プール上限
 * (RECALL_CANDIDATE_POOL)導入**前**の話。今は上位 N に絞った後で抽出するため精度は構造的に保証され、
 * 無関係混入は起きない。そこで**本来の狙い(同じ問いでも毎回同じ並びにしない揺らぎ)を取り戻す**ため
 * 0.006→0.012 へ戻す(2026-06-24・④B)。プール上限が precision を守るので温度は揺らぎ優先でよい。
 */
export const RECALL_SOFTMAX_TEMP = 0.012;
...
/**
 * 同じ気にかけを「自分から」持ち出してよい上限回数(2026-06-14 ユーザー決定)。
 * **1=「一度聞いて答えが無ければ、もう自分からは蒸し返さない」**(人間の自然な引き際)。
 * 上限に達した loop は気にかけ注入(会話・自発発話の両経路)から外れる=休眠。
 * ただし**関連する話題が会話に出れば想起(retriever)経路で自然に再浮上する**ため、
 * 「そういえばあの件どうなった?」という良い再訪は残る(休眠=自発的な蒸し返しの停止のみ)。
 */
export const OPEN_LOOP_MAX_SURFACES = 1;
