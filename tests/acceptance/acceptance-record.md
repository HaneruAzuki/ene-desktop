...
> ⚠️ **本記録は 0.1.0(MVP・2026-06-03)時点の受入結果である。** その後の post-MVP 開発
> (音声・VRM 3D 表示・UI 改修・レイテンシ最適化・忘却・存在感の改修。CHANGELOG の `[Unreleased]` 参照)は
> 本記録に含まれない。テスト規模も当時の 175 から 507(2026-06-16 時点)へ増えている。
> post-MVP 機能の受入(特に成功基準8 と体感を伴う手動項目=人間判定必須)は **別途の受入パスが必要**であり、
> 本記録を現行版の合格証跡として扱ってはならない。
