...
describe('token-counter (要件 NF-PERF-06〜08)', () => {
  ...
  it('estimatePromptTokens は system + 全メッセージの文字数から見積もる', () => {
  ...
  it('countAndCheck は巨大プロンプトを hard_limit で拒否する', () => {
    const r = countAndCheck({ system: 'あ'.repeat(200_000), messages: [] });
    expect(r.ok).toBe(false);
    expect(r.reason).toBe('hard_limit');
  });

  it('countAndCheck は通常サイズを許可する', () => {
    const r = countAndCheck({ system: 'ふつうの長さ', messages: [{ role: 'user', content: 'こんにちは' }] });
